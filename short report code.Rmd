...
# add strikeouts to xBA
statcast_2023 <- statcast_2023 %>%
...
# temporary data use 07-04 as an example date
data <- statcast_2023 %>%
  filter(game_date < '2023-07-04')
```

#### Historical hits response data

```{r}
game_data <- statcast_2023 %>%
...
pitcher_stats <- pitcher_90 %>%
  ...
  mutate(date = '2023-07-04') %>%
   ...
   # remove sparse columns
   select(-c("Knuckleball_R_90_pitcher", "Screwball_R_90_pitcher", "Slurve_R_90_pitcher",
            "Eephus_R_90_pitcher", "Forkball_R_90_pitcher", "Other_R_90_pitcher", "Pitch Out_L_90_pitcher", 
            "Pitch Out_R_90_pitcher", "Other_L_90_pitcher", "_L_90_pitcher", "_R_90_pitcher"))
...
```{r}
full_data <- game_data %>%
  left_join(batter_stats, by = c('batter', 'date')) %>%
  left_join(pitcher_stats, by = c('opp_starter' = 'pitcher', 'date')) %>%
